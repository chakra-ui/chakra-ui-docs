...
> 🚨 Avoid passing `onClick` handlers to icon components. If you need a clickable icon, use the [IconButton](/docs/form/icon-button) instead.

<ComponentLinks theme={{ componentName: "icon" }} github={{ package: npm ={{ "@chakra-ui>
...
To use third-party icon libraries like [`react-icons`](https://react-icons.github.io/react-icons/), here are the steps:
...
Both `Icon` and `createIcon` enable you to style the icon using [style props](/docs/features/style-props).
...
- Export icons as `svg` from [Figma](https://www.figma.com/), [Sketch](https://www.sketch.com/), etc.
- Use a tool like [SvgOmg](https://jakearchibald.github.io/svgomg/) to reduce the size and minify the markup.
