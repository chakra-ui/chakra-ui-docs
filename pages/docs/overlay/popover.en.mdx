...
<ComponentLinks theme={{ componentName: "popover" }} github={{ package: npm ={{ "@chakra-ui>
...
You can leverage [react-focus-lock](https://github.com/theKashey/react-focus-lock) to trap focus within the `PopoverContent`.
...
Since popover is powered by PopperJS, you can change the placement of the popover by passing the `placement` prop. See the [props](#props) for the possible placement values.
...
> When you see the word _"trigger"_, it is referring to the `children` of `PopoverTrigger`
