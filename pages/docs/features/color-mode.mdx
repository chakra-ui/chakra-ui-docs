...
- if `--chakra-ui-color-mode` is defined through e.g. `ColorModeScript` / after modification/initial load of the colorMode this
  value will be used
...
if `useSystemColorMode=true` we will always try to match the users `system`-color and fallback to `initialColorMode`
With this behavior the colorMode toggle won't have any effect.

if `initialColorMode='system'` we will as well always try to match the users `system`-color and fallback to `light`.
