...
Building on the last example, you may want to customise the input as well. I've also tweaked the UI, by adding a hover color, and a tooltip. As you can see, we can change the input by setting the `as` prop. This design is inspired by the [Atlassian inline edit](https://atlassian.design/components/inline-edit/examples) component.
