...
<ComponentLinks theme={{ componentName: "menu" }} github={{ package: npm ={{ "@chakra-ui>
...
To group related `MenuItem`s, use the `MenuGroup` component and pass it a `title` for the group name.
...
`MenuButton` composes [Box](/docs/layout/box) so you can pass all `Box` props to change its style.

### MenuList Props

`MenuList` composes [Box](/docs/layout/box) so you can pass all `Box` props to change its style.
...
`MenuGroup` composes [Box](/docs/layout/box) so you can pass all `Box` props to change its style.
...
`MenuItemOption` composes [Box](/docs/layout/box) so you can pass all box props in addition to these:
