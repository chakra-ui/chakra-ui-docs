...
You can decide:
 ...
 - If a change in the `system` color mode must change the app's color mode.
...
// TODO: default is undefined or false ???
...
If you prefer, you can use this [playground](https://codesandbox.io/s/chakra-ui-color-mode-test-f5fcwr?file=/src/uiLayer.chakra/customTheme.ts) to try different values.

- **A.** `system` sets initial value and can update it.
```tsx live=false
const config: ThemeConfig = {
...
- **B.** `system` sets initial value and can NOT update it.
```tsx live=false
const config: ThemeConfig = {
  initialColorMode: 'system',
  useSystemColorMode: false,
...
- **C.** You choose initial value, `system` can NOT update it.
```tsx live=false
const config: ThemeConfig = {
...
- **D.** You choose initial value, `system` can update it.
```tsx live=false
const config: ThemeConfig = {
  initialColorMode: 'dark', // 'dark' | 'light'
  useSystemColorMode: true,
