...
If `useSystemColorMode=true` and operating system changes from `light` to `dark` due to daylight switch, the page automatically changes color mode, without user interaction.
