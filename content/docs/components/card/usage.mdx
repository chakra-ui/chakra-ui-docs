...
Simply put in some content in the `CardBody` to get a basic card.
...
Card comes with an inherent `display="flex"`. This means you make the content in
a horizontal direction by passing `direction="row`.
...
Compose Card with other components like `Avatar`, `Icon Button` and more for a
