...
- `variant`: The visual variant of the badge. Defaults to `solid`.
- `colorScheme`: The color scheme of the badge. Defaults to `gray`.
- `size`: The size of the badge. Defaults to `md`.
...
After customizing the badge theme, we can import it in our theme file and add
it in the `components` property:

```jsx live=false
import { extendTheme } from '@chakra-ui/react'
import { badgeTheme } from './components/badge.ts'
