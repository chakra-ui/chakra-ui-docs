...
> [Component Style](/docs/styled-system/component-style)
> page.
...
## Theming utilities
The following examples use theme utility functions to help define the table theme. These steps are included in all of the examples below but are omitted from the specific examples to avoid repeating information.

1. Import the `tableAnatomy` object from `@chakra-ui/anatomy`
1. Import `createMultiStyleConfigHelpers`, which provides a set of utilities for creating style configs.
1. Destructure `definePartsStyle` and `defineMultiStyleConfig` from `createMultiStyleConfigHelpers`. These utility functions define style configurations for multi-part components.


## Customizing the default Table theme
1. Create a constant to store the base style utilizing the `definePartsStyle` function to create a multipart style object with your customizations.
1. Create a constant to store the table theme utilizing the `defineMultiStyleConfig` function to store the style changes for the multipart table component.
...
**Step 3 example**
> Step 3 is a crucial step to make sure that any changes that we make to the table
> theme are applied.
...
1. Create a constant to store the new size's style utilizing the `definePartsStyle` function to create a multipart style object with your new size.
1. Create a constant to store the table size style utilizing the `defineMultiStyleConfig` function to define the style configuration for the multipart table component.
1. Import the table theme into your theme file's components property.
1. Run the CLI tool to update your IDE's autocomplete recognition of the new size. Learn more about the CLI tool  [here](/docs/styled-system/cli).
...
// Now we can use the new `xl` size in our application
<Table size="xl" ... />
```
...
1. Create a constant to store the new variant's style utilizing the `definePartsStyle` function to create a multipart style object with your new variant.
1. Create a constant to store the table variant style utilizing the `defineMultiStyleConfig` function to define the style configuration for the multipart table component.
1. Import the table theme into your theme file's components property.
1. Run the CLI tool to update your IDE's autocomplete recognition of the new size. Learn more about the CLI tool  [here](/docs/styled-system/cli).
...
export const tableTheme = defineMultiStyleConfig({ variants: {rounded} })
```
...
To change the default variant and size used for every `Table` component, change the component's `defaultProps`. This creates efficiency, preventing the need to set the size or variant each time a `Table` component is used.
