...
The `IconButton` component is a single part component. All of the styling is applied
...
> To create theming just for the `IconButton` component, you can create a **custom variant**.
...
Let's assume we want to include an extra large icon button size. Here's how we can do
that:
...
Let's assume we want to create a custom `IconButton` variant. Here's how we can do
that:
