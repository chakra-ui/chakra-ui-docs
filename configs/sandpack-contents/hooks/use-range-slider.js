module.exports = {
    ...
    export default function App({ min, max, stepToNumber, stepToIndex, stepByNumber, defaultValue, ...rest }: Props) {
      const { state, actions, getInnerTrackProps, getInputProps, getMarkerProps, getRootProps, getThumbProps, getTrackProps } = useRangeSlider({ min, max, defaultValue, ...rest });
    
      const { onKeyDown: onThumbKeyDownFirstIndex, ...thumbPropsFirstIndex } = getThumbProps({
        index: 0,
      });
    
      const { onKeyDown: onThumbKeyDownSecondIndex, ...thumbPropsSecondIndex } = getThumbProps({
        index: 1,
      });
    
      const markers = Array.from({ length: 3 }, (_, i) => i + 1).map((i) => getMarkerProps({ value: i * 25 }));
      ...
      const onKeyDownStepBy = (e: React.KeyboardEvent<HTMLDivElement>, thumbIndex: number) => {
        ...
        else thumbIndex === 0 ? onThumbKeyDownFirstIndex?.(e) : onThumbKeyDownSecondIndex?.(e);
      };
      ...
      return (
        <Box px={8} pt="15%">
          <Flex flexDir="row" justifyContent="space-between">
            ...
            <Actions actions={actions} min={defaultValue[0]} max={defaultValue[1]} stepToIndex={stepToIndex} stepToNumber={stepToNumber} />
          </Flex>
          <chakra.div mt={2} cursor="pointer" w={{ base: "96%", lg: "98%" }} ml={{ base: "2%", lg: "1%" }} {...getRootProps()}>
            ...
            {markers.map((markerProps, index) => {
              ...
              return (
                <Badge ml="-18px" mt="25px" fontSize="sm" color="black" {...markerProps}>
                  {value}
                </Badge>
              );
            })}
            <Box h="7px" bgColor="teal.100" borderRadius="full" {...getTrackProps()}>
              <Box h="7px" bgColor="teal.500" borderRadius="full" {...getInnerTrackProps()} />
            </Box>
            <Thumb value={state.value[0]} thumbIndex={0} thumbProps={thumbPropsFirstIndex} onKeyDownStepBy={onKeyDownStepBy} bgColor="teal.500" />
...
export default ThumbIndexIcon;`,
    ...
    const Thumb = ({ value, bgColor, thumbIndex, thumbProps, onKeyDownStepBy }: Props) => {
      return (
        <Box
          top="1%"
          boxSize={8}
