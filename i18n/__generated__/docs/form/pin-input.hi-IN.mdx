...
description: The PinInput component is similar to the Input component, but is optimized for entering sequences of digits quickly. ---
---

The `PinInput` component is similar to the [Input](/docs/form/input) component, but it is optimized for entering sequences of digits.

The most common application is for entering OTP or security codes.

<ComponentLinks theme={{ componentName: "pin-input" }} github={{ package: npm ={{ "@chakra-ui>
...
By default, `PinInput` moves focus automatically to the next input once a field is filled. It also transfers focus to a previous input when <kbd>Delete</kbd> is pressed with focus on an empty input.
