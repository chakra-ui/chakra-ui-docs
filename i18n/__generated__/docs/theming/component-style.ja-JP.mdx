...
Since the new Card component is **not** part of Chakra UI we need to create a new React component and consume the style we just created. We can do that using `useStyleConfig` hook.
...
> Please note that we are passing the styles to the prop `__css`. It has the same API as [the `sx` prop](/docs/features/the-sx-prop), but has a lower style priority. This means you can override the style properties with chakra style props.
...
> **Pro tip 💡**: If you're looking for a list of parts of a multipart component you can check it by clicking on the **"View theme source"** button at the top of the documentation page for that certain component. Check out [this](https://chakra-ui.com/docs/overlay/modal) example.
